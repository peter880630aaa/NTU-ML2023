...
def parse_args():
    ...
    parser.add_argument(
        "--train_file", type=str, default="hw7_train.json", help="A csv or a json file containing the training data."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--validation_file", type=str, default="hw7_dev.json", help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
    ...
    parser.add_argument("--seed", type=int, default=4586, help="A seed for reproducible training.")
    parser.add_argument(
        "--doc_stride",
        type=int,
        default=128,
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        extension = "json"
        raw_datasets = load_dataset(extension, data_files=data_files, field="questions")
    ...
    def org2squad(example):
        squad = {}
        squad["id"] = example["id"]
        squad["question"] = example["question_text"]
        squad["context"] = train_paragraphs[example["paragraph_id"]]
        answers = {}
        answers["answer_start"] = [example["answer_start"]]
        answers["text"] = [example["answer_text"]]
        squad["answers"] = answers
        return squad

    def org2squad2(example):
        squad = {}
        squad["id"] = example["id"]
        squad["question"] = example["question_text"]
        squad["context"] = dev_paragraphs[example["paragraph_id"]]
        answers = {}
        answers["answer_start"] = [example["answer_start"]]
        answers["text"] = [example["answer_text"]]
        squad["answers"] = answers
    ...
    raw_datasets["train"] = raw_datasets["train"].map(
        org2squad, remove_columns=raw_datasets["train"].column_names
    )

    raw_datasets["validation"] = raw_datasets["validation"].map(
        org2squad2, remove_columns=raw_datasets["validation"].column_names
    )
